defmodule DoormanWeb.DoorTestHelpers do
  ...
  def add_door(%{user: user}) do
    {:ok, door} = Access.create_door(@create_door_attrs, user)
    {:ok, %{door: door}}
  end

  def add_other_door(%{other_user: user}) do
    {:ok, door} = Access.create_door(@create_other_door_attrs, user)
    {:ok, %{other_door: door}}
  end

  def add_open_grant(%{door: door}) do
    {:ok, grant} = Access.add_door_grant(door, %{timeout: one_hour_from_now()})
    {:ok, %{grant: grant}}
  end

  def add_closed_grant(%{door: door}) do
    {:ok, grant} = Access.add_door_grant(door, %{timeout: now()})
    {:ok, %{grant: grant}}
  end

  def add_other_grant(%{other_door: other_door}) do
    {:ok, grant} = Access.add_door_grant(other_door, %{timeout: ~U[2011-05-18 15:01:01Z]})
